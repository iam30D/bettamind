...
| iOS SQLCipher dependency route or native link fails on macOS | Phase 3 cannot prove encrypted SQLite on iOS | Use official `SQLCipher.swift` pinned to `4.16.0`, checksum-verify the XCFramework for Gradle cinterop, link the package in Xcode, and validate through Codemagic `ios-simulator-unsigned` | Pending validation |
| Phase 4 flow appears to store personal narrative before encrypted storage is ready | Privacy and trust failure | Keep Phase 4 deterministic and in-memory only; show encrypted-storage status and disable narrative persistence until the encrypted storage route is complete | Active |
